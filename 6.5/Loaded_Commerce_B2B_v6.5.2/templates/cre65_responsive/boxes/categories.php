...
*/
class bootstrap_box_categories {
  public $row_count = 0;
  ...
  public function __construct() {
    ...
    if (tep_not_null($cPath)) {
      ...
      while (list($key, $value) = each($cPath_array)) {
        unset($parent_id);
        unset($first_id);
        $query = tep_db_query("SELECT c.categories_id, 
                                      cd.categories_name, 
                                      c.parent_id 
                                 FROM " . TABLE_CATEGORIES . " c, 
                                      " . TABLE_CATEGORIES_DESCRIPTION . " cd 
  ...
  private function build_categories_string($counter) {
    ...
    for ($i=0; $i<$this->tree[$counter]['level']; $i++) {
      $this->categories_string .= "<i class=\"fa fa-angle-right padding-left-5 padding-right-10\"></i>";
    }
...
$categories = new bootstrap_box_categories();  
?>
<!-- categories //-->
<div class="categories-infobox">
  <div class="box-header infoBoxHeading infoBoxHeadingCenter">
    <?php echo BOX_HEADING_CATEGORIES; ?>
  </div>
  <div class="row margin-top-10 margin-bottom-10">
    <div class="col-lg-12">
      <ul class="box-product-categories-ul">
