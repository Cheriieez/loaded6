...
class upsxml {
  ...
  function upsxml() {
    ...
    if (($this->enabled == true) && ((int)MODULE_SHIPPING_UPSXML_RATES_ZONE > 0)) {
      $check_flag = false;
      $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_UPSXML_RATES_ZONE . "' and ( zone_country_id = 0 or zone_country_id = '" . $order->delivery['country']['id'] . "' ) order by zone_id");
  ...
  function quote($method = '') {
	...
	$destcity	=	$order->delivery['city'];
	$deststate	=	$state;
	if(basename($_SERVER['PHP_SELF']) == 'shopping_cart.php' && $_SESSION['customer_id']=='')
	{
		include('Googlemaps.php');
		$google_maps = new Googlemaps();
		//$find_us_add = $google_maps->get_address($_POST['zip_code']);
		$destcity	=	$google_maps->get_city($_POST['zip_code']);
		$deststate	=	$google_maps->get_state($_POST['zip_code']);
	}
    $this->_upsDest($destcity, $deststate, $order->delivery['country']['iso_code_2'], $order->delivery['postcode']);
    ...
    if ((is_array($upsQuote)) && (sizeof($upsQuote) > 0)) {
	  ...
	  /*if($shipping_weight <= $max_weight_res['configuration_value'])
	  {
      	$this->quotes['methods'] = $methods;
	  }
	  else
	  {	
	  	$this->quotes['error'] = $max_weight_error_res['configuration_value'];
      }*/
	} else {
      if ( $upsQuote != false ) {
        $errmsg = $upsQuote;
	  ...
	  $ship_error = tep_db_fetch_array(tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'shipping_error'"));
      ...
      $errmsg .= '<br>' .  $ship_error['configuration_value'];
      $this->quotes = array('module' => $this->title, 'error' => $errmsg);
    }
    if (tep_not_null($this->icon)) {
      $this->quotes['icon'] = tep_image($this->icon, $this->title);
    }
	//echo '<pre>'; print_r($this->quotes);
    return $this->quotes;
  ...
  function _addItem($length, $width, $height, $weight, $price = 0 ) {
    ...
    if ((float)$weight < 1.0 && (float)$weight > 0.0) {
      $weight = 1;
    } else {
  ...
  function exclude_choices($type) {
    ...
    for ($za = 0; $za < count ($disallowed_types); $za++ ) {
	  ...
	  //echo constant('UPSXML_' . trim($disallowed_types[$za]));
