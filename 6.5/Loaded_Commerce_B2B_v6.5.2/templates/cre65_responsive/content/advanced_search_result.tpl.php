...
function keyword_replace($key) {
	...
	for ($i = 0; $i < count($keys); $i++) {
		if ($i==0) {
			$new_keyword .= str_replace("'", "&#39;", tep_db_prepare_input($keys[$i]));
		} else {
			$new_keyword .= ' ' . str_replace("'", "&#39;", tep_db_prepare_input($keys[$i]));
		}
...
*/
 ...
 IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price,
 IF(s.status, s.specials_new_products_price, p.products_price) as final_price,
 pd.products_name as relevance,
 p.products_model as relevance2 ";
    ...
    switch ($search_keywords[$i]) {
      ...
      default:
        $keyword = str_replace("'", "&#39;", tep_db_prepare_input($search_keywords[$i]));
		    ...
		    if (count($splitkeyword) > 1) {
          $cnt = 0;
          foreach ($splitkeyword as $kwd) {
            if ($cnt > 0) {
              $where_str .= " AND ";
            }
            $where_str .= "(p.products_model like '%" . (ltrim(rtrim($kwd, '"'), '"')) . "%' OR 
                           p.sub_products_data like '%" . (ltrim(rtrim($kwd, '"'), '"')) . "%' OR 
                           pd.products_name like '%" . (ltrim(rtrim($kwd, '"'), '"')) . "%' OR 
                           pd.additional_info like '%" . (ltrim(rtrim($kwd, '"'), '"')) . "%' OR 
                           pd.products_description like '%" . (ltrim(rtrim($kwd, '"'), '"')) . "%')";
            $cnt++;
          }
          //die();
		  	  /*$newkeyword	=	'';
          ...
          //$where_str .= " OR MATCH(p.products_model,p.product_attribute_data,p.sub_products_data) AGAINST('" . ($newkeyword) . "' IN BOOLEAN MODE)";
          //$where_str .= " OR p.products_model like '%" . ($modelkeyword) . "%' OR p.sub_products_data like '%" . ($modelkeyword) . "%'";
          $where_str .= "p.products_model like '%" . ($modelkeyword) . "%' OR 
                         p.sub_products_data like '%" . ($modelkeyword) . "%' OR 
                         pd.products_name like '%" . ($modelkeyword) . "%' OR 
                         pd.additional_info like '%" . ($modelkeyword) . "%' OR 
                         pd.products_description like '%" . ($modelkeyword) . "%'";
		    }
        break;
    }
  }
