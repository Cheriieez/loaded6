...
<!-- shipping_estimator //-->
          <tr>
            <td align="<?php echo (strstr($PHP_SELF,'shipping_estimator.php') ? 'right' : 'center'); ?>" style="border:none;">
...
// shopping cart quotes
  ...
  $info_box_contents[] = array('text' => '<div class="sideblocks"><h1 class="col-lg-12 gry_box2 y_clr con_txt">'.SHIPPING_OPTIONS.'</h1></div>');
...
//  $ShipTxt= tep_draw_form('estimator', tep_href_link(FILENAME_SHOPPING_CART, '', 'NONSSL'), 'post'); //'onSubmit="return check_form();"'
  $ShipTxt= tep_draw_form('estimator', tep_href_link(basename($PHP_SELF), '', 'NONSSL'), 'post','class="form-horizontal"'); //'onSubmit="return check_form();"'
  $ShipTxt.='<table>';
  if(sizeof($quotes)) {
    if ( isset($_SESSION['customer_id']) ) {
      // logged in
      ...
      $ShipTxt.='<tr><td colspan="3" class="main"><div class="form-group"><label for="inputPassword3" class="col-sm-3 control-label">Qty & Weight:</label><div class="col-sm-9" style="padding-top:7px;">' . ($total_count == 1 ?  SHIPPING_METHOD_ITEM  :  SHIPPING_METHOD_ITEMS) . $total_count . '&nbsp;&nbsp;' . SHIPPING_METHOD_WEIGHT . $total_weight . SHIPPING_METHOD_WEIGHT_UNIT . '</div></div></td></tr>';
      $ShipTxt.='<tr><td colspan="3" class="main" nowrap><div class="form-group"><label for="inputPassword3" class="col-sm-3 control-label">' .
                SHIPPING_METHOD_ADDRESS .'</label><div class="col-sm-9">'. tep_draw_pull_down_menu('address_id', $addresses_array, $selected_address, 'onchange="document.estimator.submit();return false;" class="form-control"').'</div></div></td></tr>';
      $ShipTxt.='<tr valign="top"><td class="main" colspan="3"><div class="form-group"><label for="inputPassword3" class="col-sm-3 control-label">' . SHIPPING_METHOD_TO .'</label><div class="col-sm-9">'. tep_address_format($order->delivery['format_id'], $order->delivery, 1, ' ', '<br>') . '</div></div></td></tr>';
    } else {
      ...
      $ShipTxt.='<tr><td colspan="3" class="main" nowrap><div class="form-group"><label for="inputEmail3" class="col-sm-2 control-label">' .
                ENTRY_COUNTRY .'</label><div class="col-sm-10">'. tep_get_country_list('country_id', $selected_country,'style="width=200;" class="form-control"').'</div></div>';
      if(SHIPPING_METHOD_ZIP_REQUIRED == "true"){
        $ShipTxt.='</td></tr>          <tr>
          </tr><tr><td colspan="3" class="main" nowrap><div class="form-group"><label for="inputEmail3" class="col-sm-2 control-label">'.ENTRY_POST_CODE .'</label><div class="col-sm-10">'. tep_draw_input_field('zip_code', $selected_zip, 'size="10" class="form-control"').'</div></div>';
      }
      $ShipTxt.='<div class="form-group"><div class="col-sm-offset-2 col-sm-10"><a href="_" class="btn btn-danger" onclick="document.estimator.submit();return false;">Calculate</a></div></div></td></tr>';
    }
    if (defined('MVS_STATUS') && MVS_STATUS == 'true') {   
    ...
    if ($free_shipping==1) {
      ...
      $ShipTxt.='<tr><td></td><td class="main" align="left"><b>' . SHIPPING_METHOD_TEXT . '</b></td><td class="main" align="center"><b>' . SHIPPING_METHOD_RATES . '</b></td></tr>';
      $ShipTxt.='<tr><td colspan="3" class="main">'.tep_draw_separator().'</td></tr>';
      //echo 'here im<pre>'; print_r($quotes);
        ...
        if (sizeof($quotes[$i]['methods'])==1) {
          ...
          if(isset($quotes[$i]['error'])){
            ...
            $ShipTxt.= '('.$quotes[$i]['error'].')</td></tr>';
		  	...
		  	if($quotes[$i]['module']=='Ground Shipping')
			{
