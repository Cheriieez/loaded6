...
class box_articles {
  ...
  public function __construct() {
    ...
    $topic_group_access = tep_get_access_sql('t.products_group_access', $customer_group_array);
    $query = tep_db_query("SELECT t.topics_id, td.topics_name, t.parent_id
                           FROM " . TABLE_TOPICS . " t,
                                " . TABLE_TOPICS_DESCRIPTION . " td
                           WHERE t.parent_id = 0
                             and t.topics_id = td.topics_id
                             and td.language_id = " . (int)$languages_id . $topic_group_access . "
    ...
    if (tep_not_null($tPath)) {
      ...
      if (is_array($tPath_array)) {
        reset($tPath_array);
        $group_access_tPath = tep_get_access_sql('t.products_group_access', $customer_group_array);
        while (list($key, $value) = each($tPath_array)) {
          ...
          $query = tep_db_query("SELECT t.topics_id, td.topics_name, t.parent_id
                                 ...
                                 WHERE t.parent_id = " . (int)$value . "
                                   and t.topics_id = td.topics_id
                                   and td.language_id = " . (int)$languages_id . $group_access_tPath . "
                                 ORDER BY sort_order, td.topics_name");
      ...
      if (SHOW_ARTICLE_COUNTS == 'true') {  
        $articles_new_query = tep_db_query("SELECT a.articles_id
                                            ...
                                            WHERE a.articles_status = 1
                                              ...
                                              and td.language_id = " . (int)$languages_id . $group_access_articles . "
