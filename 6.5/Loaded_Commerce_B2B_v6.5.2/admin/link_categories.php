  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'update':
        ...
        $sql_data_array = func_group_access_array($sql_data_array,(($action == 'insert') ? 'new' : ''));
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          ...
          if (isset($_POST['Push'])){
             // if($_POST['Push']==2){
                  // we do not have sub category concept in links.
             // }
              if($_POST['Push']==3){
                  tep_db_query("UPDATE " . TABLE_LINKS . " l, " . TABLE_LINKS_TO_LINK_CATEGORIES . " l2c set products_group_access = '" . $customergroupid . "'  where l.links_id = l2c.links_id and l2c.link_categories_id= '" . (int)$link_categories_id . "'");
              }
          }  
        ...
        if( !empty($link_categories_image)  ) {
...
<table border="0" width="100%" cellspacing="0" cellpadding="0" class="body-table">
  ...
  switch ($action) {
   case 'new':
    $heading_title = 'New Links Category';
    $form_action = 'insert';
    break;

   case 'edit':
    $heading_title = 'Edit Links Category';
    $form_action = 'update';
    break;
...
<?php
  ...
  switch ($action) {
   case 'new':
    case 'edit':
echo tep_draw_form('new_link_categories', FILENAME_LINK_CATEGORIES, 'action=' . $form_action, 'post', 'enctype="multipart/form-data"');
if($action == 'edit'){
    echo tep_draw_hidden_field('link_categories_id', $cInfo->link_categories_id);
}
?>
...
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  <tr>
    <td><?php echo TEXT_LINK_CATEGORIES_IMAGE;?></td>
    <td><?php echo tep_draw_file_field('link_categories_image');?></td>
  </tr>
<?php
if(tep_not_null($cInfo->link_categories_image)){
?>
  <tr>
    <td>&nbsp;</td>
    <td><?php echo tep_info_image($cInfo->link_categories_image, $cInfo->link_categories_name) . ' &nbsp;' . $cInfo->link_categories_image;?></td>
  </tr>
<?php
}
?>
  <tr>
    <td><?php echo TEXT_LINK_CATEGORIES_SORT_ORDER;?></td>
    <td><?php echo tep_draw_input_field('link_categories_sort_order', $cInfo->link_categories_sort_order, 'size="2"');?></td>
  </tr>
  <tr>
    <td><?php echo TEXT_LINK_CATEGORIES_STATUS;?></td>
    <td><?php echo '<table><tr><td>'.tep_draw_radio_field('link_categories_status', 'on', ($cInfo->link_categories_status == '1') ? true : false).'</td><td>'.TEXT_LINK_CATEGORIES_STATUS_ENABLE.'</td><td width="10">&nbsp;</td><td>'.tep_draw_radio_field('link_categories_status', 'off', ($cInfo->link_categories_status == '0') ? true : false).'</td><td>'.TEXT_LINK_CATEGORIES_STATUS_DISABLE.'</td></tr></table>';?></td>
  </tr>
</table>
</td>
    <td width="50%" valign="top"><div class="tab-pane" id="tabPane1">
...
<table width="100%" border="0" cellspacing="2" cellpadding="1">
  <tr>
    <td valign="top"><?php echo TEXT_LINK_CATEGORIES_NAME . '</td><td>' . tep_draw_input_field('link_categories_name[' . $languages[$i]['id'] . ']', tep_get_link_category_name($cInfo->link_categories_id, $languages[$i]['id']));?><br />
  </tr>
  <tr>
    <td valign="top"><?php echo TEXT_LINK_CATEGORIES_DESCRIPTION  . '</td><td>'  .tep_draw_textarea_field('link_categories_description[' . $languages[$i]['id'] . ']', 'soft', '40', '5', tep_get_link_category_description($cInfo->link_categories_id, $languages[$i]['id']));?></td>
  </tr>
</table>
        <?php 
           } 
      ...
      <script type="text/javascript">setupAllTabs();</script></td>
  </tr>
  <tr>
    <td colspan="2" valign="top"><?php
                if (!isset($cInfo->link_categories_id)) {
                   $current_category_id = 0;
                 } else {
                   $current_category_id = $cInfo->link_categories_id;
                 }
            ...
            echo func_category_group_access($group_access,$current_category_id);
?></td>
  </tr>
  <tr>
    <td valign="top">&nbsp;</td>
    <td align="right" valign="top"><?php echo '<a href="' . tep_href_link(FILENAME_LINK_CATEGORIES) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>' . tep_image_submit('button_save.gif', IMAGE_SAVE);?></td>
  </tr>
</table>
</form>
</td></tr>
