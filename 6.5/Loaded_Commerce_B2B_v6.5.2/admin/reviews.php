...
if (tep_not_null($action)) {
  switch ($action) {
    ...
    case 'deleteconfirm':
      ...
      // Added for Points & Rewards START - maestro
      if ('MODULE_ADDONS_POINTS_STATUS' == 'True') {
        if ($_GET['uID'] == '') {
          tep_db_query("UPDATE " . TABLE_CUSTOMERS . " SET customers_shopping_points = '" . $_GET['nSP'] . "' WHERE customers_id = '" . $_GET['cID'] . "'");
        }
        if (isset($_POST['delete_rp']) && ($_POST['delete_rp'] == 'on')) {
          tep_db_query("DELETE FROM " . TABLE_CUSTOMERS_POINTS_PENDING . "  where unique_id = '" . $_GET['uID'] . "'");
        }
      }
      // Added for Points & Rewards END - maestro
        ...
        <tr>
          <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
            <tr>
              ...
              //Added for Points & Rewards START - maestro
              if ('MODULE_ADDONS_POINTS_STATUS' == 'True') {
                $cust_id_query = tep_db_query("select customers_id from " . TABLE_REVIEWS . " where reviews_id = '" . (int)$rInfo->reviews_id . "'");
                $cust_id = tep_db_fetch_array($cust_id_query);
                $cust_id = $cust_id['customers_id'];
                $uID_query = tep_db_query("SELECT unique_id FROM " . TABLE_CUSTOMERS_POINTS_PENDING . " WHERE orders_id = '" . (int)$rInfo->products_id . "' and customer_id = '" . $cust_id . "' and date_added = '" . $rInfo->date_added . "'");
                $uID_result = tep_db_fetch_array($uID_query);
                $uID = $uID_result['unique_id'];
                $current_shopping_points_query = tep_db_query("SELECT customers_shopping_points FROM " . TABLE_CUSTOMERS . " WHERE customers_id = '" . (int)$cust_id . "'");
                $current_shopping_points = tep_db_fetch_array($current_shopping_points_query);
                if ($uID != '') {
                  $new_shopping_points = $current_shopping_points['customers_shopping_points'];
                } else {
                  $new_shopping_points = ($current_shopping_points['customers_shopping_points'] - USE_POINTS_FOR_REVIEWS);  
                }
              }
              //Added for Points & Rewards END - maestro
              switch ($action) {
                ...
                //Modified for Points & Rewards START
                  $contents = array('form' => tep_draw_form('reviews', FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&cID=' . $rInfo->customers_id . '&nSP=' . $new_shopping_points . '&uID=' . $uID . '&action=deleteconfirm'));
                //Modified for Points & Rewards END
                ...
                //Added for Points & Rewards START
                  if (MODULE_ADDONS_POINTS_STATUS == 'True') {
                    if ($uID != '') {
                      $contents[] = array('text' => '<hr /><br />' . tep_draw_checkbox_field('delete_rp', '', false) . ' ' . TEXT_DELETE_REVIEW_POINTS_START . TEXT_DELETE_REVIEW_POINTS_END);
                    }
                  }
                //Added for Points & Rewards END
                  break;
                default:
                  if (isset($rInfo) && is_object($rInfo)) {
                    $heading[] = array('text' => '<b>' . $rInfo->products_name . '</b>');
                  //Modified for Points & Rewards START
                    $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&cID=' . $cust_id . '&nSP=' . $new_shopping_points . '&uID=' . $uID . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
                  //Modified for Points & Rewards END
