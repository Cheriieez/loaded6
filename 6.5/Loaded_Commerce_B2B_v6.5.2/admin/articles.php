    ...
    switch ($action) {
      ...
      case 'update_topic':
        if ( isset($_POST['edit_x']) || isset($_POST['edit_y']) ) {
          ...
          /*    $customergroupidcnt=$_POST['customergroupidcnt'];
          $customergroupid="";
          for ($i=0; $i <= $customergroupidcnt; $i++)
          {
          if($_POST['customergroupid'.$i]!='N')
          {
          if($_POST['customergroupid'.$i]=='Y')
          {$customergroupid.="G,";}
          else{$customergroupid.=$_POST['customergroupid'.$i].",";}
          }
          }
          $customergroupid=substr($customergroupid,0,(strlen($customergroupid)-1));

          if($action == 'new_topic' && $customergroupid=="")
          {
          $customergroupid="G,0";
          }

          $arrcustomergroupid=array('products_group_access' => $customergroupid);
          $sql_data_array = array_merge($sql_data_array, $arrcustomergroupid);*/
          ...
          for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
            ...
            if ($action == 'insert_topic') {
              ...
              if (isset($_POST['Push'])){
                if($_POST['Push']==2)
                {readTopicChild($topics_id,$customergroupid,"C");}

                if($_POST['Push']==3)
                {readTopicChild($topics_id,$customergroupid,"CP");}
              }   
              /************   GSR - Group Access Logic - End   ************/
      ...
      case 'update_article':
        if (isset($_POST['edit_x']) || isset($_POST['edit_y'])) {
          ...
          if ($action == 'insert_article' && $customergroupid=="")
          {
          $customergroupid="G,0";
          }
          ...
          */
          ...
          if ($action == 'insert_article') {
            ...
            if (isset($_POST['articles_date_added']) && tep_not_null($_POST['articles_date_added'])) {
              $insert_sql_data = array('articles_date_added' => tep_db_prepare_input($_POST['articles_date_added']));
            } else {
            ...
            if (isset($_POST['articles_date_available']) && tep_not_null($_POST['articles_date_available'])) {
              $insert_sql_data = array('articles_date_available' => tep_db_prepare_input($_POST['articles_date_available']));
            } else {
              $insert_sql_data = array('articles_date_available' => 'now()');
            }
    ...
    <div id="body">
      <table border="0" width="100%" cellspacing="0" cellpadding="0" class="body-table">
          ...
          <td class="page-container" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="0">
              <?php
                ...
                if (isset($_GET['action']) && ($_GET['action'] == 'new_topic_ACD' || $_GET['action'] == 'edit_topic_ACD')) {
                  if ( isset($_GET['tID']) && (!$_POST) ) {
                    $topics_query = tep_db_query("select t.topics_id, td.topics_name, td.topics_heading_title, td.topics_description, t.parent_id, t.sort_order, t.date_added, t.last_modified,t.products_group_access from " . TABLE_TOPICS . " t, " . TABLE_TOPICS_DESCRIPTION . " td where t.topics_id = '" . $_GET['tID'] . "' and t.topics_id = td.topics_id and td.language_id = '" . $languages_id . "' order by t.sort_order, td.topics_name");
                  ...
                  $text_new_or_edit = (isset($_GET['action']) && $_GET['action']=='new_topic_ACD') ? TEXT_INFO_HEADING_NEW_TOPIC : TEXT_INFO_HEADING_EDIT_TOPIC;
                ?>
                <?php echo tep_draw_form('new_topic', FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . (isset($_GET['tID']) ? $_GET['tID'] : 0) . '&action=new_topic_preview', 'post', 'enctype="multipart/form-data"'); ?>
              ...
              <table border="0" cellspacing="0" cellpadding="0" width = "100%">
                ...
                <tr>
                  ...
                  <td class="main"> --><?php echo tep_draw_separator('pixel_trans.gif', '10', '15') . '&nbsp;' . tep_draw_input_field('sort_order', (isset($tInfo->sort_order) ? $tInfo->sort_order : ''), 'size="2"'); ?></td>
                </tr>
                <tr>
                  <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                </tr>
                ...
                <tr>
                  <td colspan = "2">    
                    <?php
                      ...
                      if (!isset($tInfo->products_group_access)) {
                        $tInfo->products_group_access = "G,0";
                      }
                      $group_access = $tInfo->products_group_access;
                      $current_category_id = $current_topic_id;
              ...
              <?php 
                ...
                if(ARTICLE_WYSIWYG_ENABLE == 'Enable' ){  
                  ...
                  if (!isset($topics_description_elements)) {
                    $topics_description_elements = '';
                  }
                    ...
                    <table width="100%"  border="0" cellspacing="0" cellpadding="0" summary="tab table">
                      <tr>
                        <td valign="top"><table border="0" cellspacing="0" cellpadding="2" width="100%">
                            <tr>
                              <td class="main"><?php echo TEXT_EDIT_TOPICS_NAME; ?><?php echo '&nbsp;' . tep_draw_input_field('topics_name[' . $languages[$i]['id'] . ']', (isset($topics_name[$languages[$i]['id']]) ? stripslashes($topics_name[$languages[$i]['id']]) : (isset($tInfo->topics_id) ? tep_get_topic_name($tInfo->topics_id, $languages[$i]['id']) : ''))); ?></td>
                            </tr>
                            <tr>
                              <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                            </tr>
                            <tr>
                              <td class="main"><?php echo TEXT_EDIT_TOPICS_HEADING_TITLE; ?><?php echo '&nbsp;' . tep_draw_input_field('topics_heading_title[' . $languages[$i]['id'] . ']', (isset($topics_name[$languages[$i]['id']]) ? stripslashes($topics_name[$languages[$i]['id']]) : (isset($tInfo->topics_id) ? tep_get_topic_heading_title($tInfo->topics_id, $languages[$i]['id']) : ''))); ?></td>
                            </tr>
                            <tr>
                              <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                            </tr>
                            ...
                            <tr>
                              <td colspan="2" class="main"><?php echo tep_draw_textarea_field('topics_description[' . $languages[$i]['id'] . ']', 'soft', '70', '20', (isset($topics_description[$languages[$i]['id']]) ? stripslashes($topics_description[$languages[$i]['id']]) : (isset($tInfo->topics_id) ? tep_get_topic_description($tInfo->topics_id, $languages[$i]['id']) : '')),''); ?></td>
                            </tr>
                          </table>
                        </td>
                      </tr>
        ...
        <table border="0" cellspacing="0" cellpadding="2" width="90%">
          ...
          <tr>
            <td class="main" align="right"><?php echo tep_draw_hidden_field('topics_date_added', (isset($tInfo->date_added) ? $tInfo->date_added : date('Y-m-d'))) . tep_draw_hidden_field('parent_id', (isset($tInfo->parent_id) ? $tInfo->parent_id : '')) . tep_image_submit('button_magnifier.png', IMAGE_PREVIEW) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . (isset($_GET['tID']) ? $_GET['tID'] : '')) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
            </form></tr>
          <?php
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="2">
          ...
          <tr>
            <td><table border="0" cellspacing="0" cellpadding="2">
                  ...
                  <td class="main">
                    <table>
                      <tr>
                        <td>
                          <?php
                            echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_radio_field('articles_status', '0', $out_status);
                          ?>
                        </td>
                        <td>
                          <?php echo TEXT_ARTICLE_NOT_AVAILABLE;?>
                        </td>
                        <td width = "25">&nbsp;                    
                        </td>
                        <td>
                          <?php echo tep_draw_radio_field('articles_status', '1', $in_status);?>
                        </td>
                        <td>
                          <?php echo TEXT_ARTICLE_AVAILABLE;?>
                        </td>
                      </tr>
                    </table>
                    <?php /*echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_radio_field('articles_status', '0', $out_status) . '&nbsp;' . TEXT_ARTICLE_NOT_AVAILABLE . '&nbsp;' . tep_draw_radio_field('articles_status', '1', $in_status) . '&nbsp;' . TEXT_ARTICLE_AVAILABLE; */?>
                ...
                <!-- /************   GSR - Group Access Logic - Start   ************/ -->
                <tr>
                  <td class="main" colspan = "2" width = "350">
                    <?php
                      if (!isset($aInfo->products_group_access)) {
                        $aInfo->products_group_access = 'G,0';
                      }
                      $group_access = $aInfo->products_group_access;
                      func_product_group_access($group_access);              
                    ?>
                  </td>
                </tr>
                <tr>
              ...
              <?php 
                ...
                if(ARTICLE_WYSIWYG_ENABLE == 'Enable' ){  
                  echo tep_load_html_editor();
                  if(!isset($articles_description_elements)) {
                    $articles_description_elements = '';
                  }
                  for ($i=0; $i<sizeof($languages); $i++) {
          ...
          <tr>
            ...
            <tr>
              ...
              <?php
                ...
                ?>
                <td class="dataTableContent" style="padding:8px;"><?php echo '<a href="' . tep_href_link(FILENAME_ARTICLES, tep_get_topic_path($topics['topics_id'])) . '">' . tep_image(DIR_WS_ICONS . 'folder.png', ICON_FOLDER) . '</a>&nbsp;<b>' . $topics['topics_name'] . '</b>'; ?></td>
                <td class="dataTableContent" style="padding:8px;" align="center">&nbsp;</td>
                <td class="dataTableContent" style="padding:8px;" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $_GET['tPath'] . '&tID=' . $tInfo->topics_id . '&action=edit_topic') . '">' . tep_image(DIR_WS_ICONS . 'page_edit.png', ICON_EDIT) . '</a>'; ?>&nbsp;&nbsp;&nbsp;<?php if (isset($tInfo) && is_object($tInfo) && ($topics['topics_id'] == $tInfo->topics_id) ) { echo tep_image(DIR_WS_IMAGES . 'arrow_right_blue.png', ''); } else { echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $topics['topics_id']) . '">' . tep_image(DIR_WS_IMAGES . 'information.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
              <?php
              }
              ...
              ?>
              <td class="dataTableContent" style="padding:8px;"><?php echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $articles['articles_id'] . '&action=article_preview&read=only') . '">' . tep_image(DIR_WS_ICONS . 'magnifier.png', ICON_PREVIEW) . '</a>&nbsp;' . $articles['articles_name']; ?></td>
              <td class="dataTableContent" style="padding:8px;" align="center">
                <?php
              ...
              ?></td>
              <td class="dataTableContent" style="padding:8px;" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $articles['articles_id'] . '&action=new_article') . '">' . tep_image(DIR_WS_ICONS . 'page_edit.png', ICON_EDIT) . '</a>'; ?>&nbsp;&nbsp;&nbsp;<?php if (isset($aInfo) && is_object($aInfo) && ($articles['articles_id'] == $aInfo->articles_id)) { echo tep_image(DIR_WS_IMAGES . 'arrow_right_blue.png', ''); } else { echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $articles['articles_id']) . '">' . tep_image(DIR_WS_IMAGES . 'information.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
            </tr>
            <?php
            }

            $tPath_back = '';
            if (isset($tPath_array) && sizeof($tPath_array) > 0) {
          ...
          <tr>
            <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                <tr>
                  ...
                  <td align="right" class="smallText"><?php if (isset($tPath_array) && sizeof($tPath_array) > 0) echo '<a href="' . tep_href_link(FILENAME_ARTICLES, $tPath_back . 'tID=' . $current_topic_id) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>&nbsp;'; if (!isset($_GET['search'])) echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&action=new_topic') . '">' . tep_image_button('button_new_topic.gif', IMAGE_NEW_TOPIC) . '</a>&nbsp;' . $add_str; ?>&nbsp;</td>
                </tr>
              </table></td>
          </tr>
          <tr>
            ...
            default:
              if ($rows > 0) {
                if (isset($tInfo) && is_object($tInfo)) { // topic info box contents
                  ...
                  $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $tInfo->topics_id . '&action=edit_topic') . '">' . tep_image_button('button_page_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $tInfo->topics_id . '&action=delete_topic') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a> <a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $tInfo->topics_id . '&action=move_topic') . '">' . tep_image_button('button_move.gif', IMAGE_MOVE) . '</a>');
                  ...
                  $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo->articles_id . '&action=new_article') . '">' . tep_image_button('button_page_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo->articles_id . '&action=delete_article') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a> <a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo->articles_id . '&action=move_article') . '">' . tep_image_button('button_move.gif', IMAGE_MOVE) . '</a> <a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo->articles_id . '&action=copy_to') . '">' . tep_image_button('button_copy_to.gif', IMAGE_COPY_TO) . '</a>');            $contents[] = array('text' => '<br>' . TEXT_DATE_ADDED . ' ' . tep_date_short($aInfo->articles_date_added));
...
<?php
  ...
  function checkTopicParentStatus($pcatid,$grouptype)
  {
    GLOBAL $groupcnt;
    if($pcatid!="")
    {
      ...
      $categories_query1 = tep_db_query("select count(topics_id) as cntcat from " . TABLE_TOPICS."  where topics_id=".$pcatid.$listing1_sql);
      ...
      $category_query = tep_db_query("select parent_id from ".TABLE_TOPICS." where topics_id=".$pcatid);
      while($category = tep_db_fetch_array($category_query))
      {
        //echo $category['parent_id'];
        if($category['parent_id']!=0)
        {
          checkTopicParentStatus($category['parent_id'],$grouptype);
        }
      }
    }
  }

  function readTopicChild($pcatid,$pcustomergroupid,$categoryorproduct)
  {
    $category_query = tep_db_query("select topics_id from ".TABLE_TOPICS." where parent_id=".$pcatid);
    while($category = tep_db_fetch_array($category_query)) {
      $sql_data_array1=array('products_group_access' => $pcustomergroupid);
      $update_sql_data = array('last_modified' => 'now()');
      $sql_data_array1 = array_merge($sql_data_array1, $update_sql_data);
      tep_db_perform(TABLE_TOPICS, $sql_data_array1, 'update', "topics_id = '" . (int)$category['topics_id'] . "'");
      if($categoryorproduct=="CP") {
        tep_db_query("update " . TABLE_ARTICLES . " a,".TABLE_ARTICLES_TO_TOPICS." b set products_group_access = '" . $pcustomergroupid . "'  where a.articles_id=b.articles_id and b.topics_id= '" . (int)$category['topics_id'] . "'");
      }
    }
    if ($categoryorproduct=="CP") {
      tep_db_query("update " . TABLE_ARTICLES . " a,".TABLE_ARTICLES_TO_TOPICS." b set products_group_access = '" . $pcustomergroupid . "'  where a.articles_id=b.articles_id and b.topics_id= '" . (int)$pcatid . "'");
    }
  }
