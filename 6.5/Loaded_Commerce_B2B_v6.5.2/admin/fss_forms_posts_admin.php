      ...
      if ( $action == 'view_entry' && is_object($fpInfo) ) {
        ...
        if(tep_db_num_rows($valuequery)>0) {
          ...
          while($fields_values = tep_db_fetch_array($valuequery)) {
            if (preg_match("/http://[^\s]/", $fields_values['forms_fields_value'])) {
                ...
                while ($forms_posts = tep_db_fetch_array($forms_posts_query)) {
                  ...
                  <td class="dataTableContent">
                    ...
                    <td class="dataTableContent" align="right">
                      <?php if (isset($_GET['pID']) && is_object($fpInfo) && ($forms_posts['forms_posts_id'] == $fpInfo->forms_posts_id || $forms_posts['forms_posts_id'] == $_GET['pID'])) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_FSS_FORMS_POSTS_ADMIN, tep_get_all_get_params(array('pID', 'action')) . 'pID=' . $forms_posts['forms_posts_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;
                    </td>
                  </tr>
                  <?php
                }
