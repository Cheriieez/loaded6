...
*/
  ...
  if (MODULE_ADDONS_ULTIMATESEO_STATUS == 'True') {
    ...
    if (preg_match("/(save|insert_form|save_form|save_system_form|save_categories|insert_categories|delete_categories_confirm|move_categories_confirm|delete_form_confirm|move_form_confirm|copy_to_form_confirm)/i", $action)) include_once('includes/reset_seo_cache.php');
  }
...
//        }
      ...
      case 'save_form':
        ...
        $forms_name = $_POST['forms_name'];
        $forms_confirmation_content = tep_db_prepare_input($_POST['forms_confirmation_content']);
        $forms_description = tep_db_prepare_input($_POST['forms_description']);
        $forms_blurb = tep_db_prepare_input($_POST['forms_blurb']);
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
