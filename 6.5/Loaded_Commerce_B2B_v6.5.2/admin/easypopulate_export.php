  ...
  switch( $dltype ){
    ...
    case 'full':
      ...
      tep_db_query("CREATE TABLE products_groups_tmp (
                  ...
                  customers_group_cost decimal( 15, 4 ) NOT NULL default '0.0000',
                  customers_group_msrp decimal( 15, 4 ) NOT NULL default '0.0000',
      ...
      while ($products_groups = tep_db_fetch_array($products_groups_query)) {
        if ($products_groups['customers_group_id'] != 0) {
          $products_groups_array = array('customers_group_id' => $products_groups['customers_group_id'],
                                         ...
                                         'customers_group_cost' => $products_groups['customers_group_cost'],
                                         'customers_group_msrp' => $products_groups['customers_group_msrp'],
...
//      'v_products_price'    => $iii++,
      ...
      $filelayout_sql = "SELECT
          ...
          pg.customers_group_cost as v_products_cost,
          pg.customers_group_msrp as v_products_msrp,
...
if ( $download == 'stream' or  $download == 'tempfile' ){
  ...
  while ($row){
    ...
    if ($price_with_tax == 'true') {
      $row['v_products_price'] = $row['v_products_price'] + ($price_with_tax * round($row['v_products_price'] * $row_tax_multiplier / 100,2));
    }
