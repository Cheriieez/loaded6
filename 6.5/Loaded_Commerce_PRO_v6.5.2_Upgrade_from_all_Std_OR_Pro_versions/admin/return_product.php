 ...
 */
 ...
 $default_refund = tep_db_fetch_array($default_refund_query);
    ...
    $messageStack->add('rmaMessage', TEXT_ERROR . ': ' . TEXT_ERROR_EMAIL, 'error');
    }
    if(isset($_GET['name_error'])){
    $name_error = $_GET['name_error'];
    $messageStack->add('rmaMessage', TEXT_ERROR . ': ' . TEXT_ERROR_CONTACT, 'error');
    }
    if(isset($_GET['qty_error'])){
    $qty_error = $_GET['qty_error'];
    $messageStack->add('rmaMessage', TEXT_ERROR . ': ' . TEXT_ERROR_QUANTITY, 'error');
    }
  ...
  if ($_GET['action']) {
    switch ($_GET['action']) {
      ...
      case 'update':
         ...
         $customers_email = strtolower(tep_db_prepare_input($_POST['customers_email_address']));
...
<script>
 ...
 function submitForm(myForm, button) {
      ...
      if (!submitDone) {
         submitDone = true;
         button.value = TEXT_PLEASE_WAIT;
         button.disabled = true;
         myForm.submit();
      }
      else {
        alert (<?php echo TEXT_ALREADY_SUBMITTED;?>);
      }
   return true;
 }
</script>
...
<?php
    if ($messageStack->size('rmaMessage') > 0) {
?>
      <tr>
        <td><?php echo $messageStack->output('rmaMessage'); ?></td>
      </tr>
<?php
}
?>
            ...
            $select_box = '<select name="support_priority"  size="' . MAX_MANUFACTURERS_LIST . '">';
             if (MAX_MANUFACTURERS_LIST < 2) {
               ...
               while ($priority_values = tep_db_fetch_array($priority_query)) {
                 ...
                 if ($default_priority['configuration_value'] ==  $priority_values['return_reason_id']) $select_box .= ' selected="selected"';
            ...
            $select_box = '<select name="refund_method"  size="' . MAX_MANUFACTURERS_LIST . '">';
             if (MAX_MANUFACTURERS_LIST < 2) {
               ...
               while ($refund_values = tep_db_fetch_array($refund_query)) {
                 ...
                 if ($default_refund['configuration_value'] ==  $refund_values['refund_method_id']) $select_box .= ' selected="selected"';
...
<!-- End refund method selection -->
    ...
    echo '              <tr>' . "\n" .
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            <td colspan="2" class="main" valign="top" align="right"><?php echo tep_image_submit('button_insert.gif', IMAGE_INSERT,'onClick="return submitForm(document.longsubmit, this)"');?></td>
          </tr>
        </table></td>
      </form></tr>
<?php
