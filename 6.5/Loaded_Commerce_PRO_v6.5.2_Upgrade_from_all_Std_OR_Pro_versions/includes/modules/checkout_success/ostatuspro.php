  ...
  class ostatuspro {
    ...
    function process() {
      ...
      if (defined('INSTALLED_VERSION_TYPE') && stristr(INSTALLED_VERSION_TYPE, 'B2B')) {
        ...
        $t = $group['customers_group_name'];
        $ok_to_process = (preg_match("/$t/i", MODULE_CHECKOUT_SUCCESS_OSTATUSPRO_GROUPS)) ? true : false;
      } else {
        $ok_to_process = true;
      }
    ...
    function install() {
     ...
     tep_db_query("CREATE TABLE IF NOT EXISTS `orders_hold_list` (
                   ...
                   KEY `holdlist_id` (`holdlist_id`)) ENGINE=MyISAM AUTO_INCREMENT=1;");
