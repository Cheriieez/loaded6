  ...
  */
  ...
  if (defined('MODULE_ADDONS_FDM_STATUS') && MODULE_ADDONS_FDM_STATUS == 'True') { 
    ...
    if (isset($_GET['products_id'])) {
      ...
      $sql_query = tep_db_query("SELECT lp.library_id, 
                                        lf.files_name, 
                                        lfde.files_descriptive_name, 
                                        fi.icon_small, 
                                        lf.files_name 
      ...
      if ($num_files_related > 0) {
        ...
        echo    '<!-- Tab ' . $tab_id . ' -->';
        echo    '<div class="tab-page" id="' . $tab_id . '">';
        echo    '<h2 class="tab">' . $tab_title . '</h2>';
        echo    '<script type="text/javascript">tp1.addTabPage( document.getElementById( "' . $tab_id . '" ) );</script>';
