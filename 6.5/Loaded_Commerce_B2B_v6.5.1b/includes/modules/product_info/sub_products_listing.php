  ...
  <td align="right"  width = "100%"><table cellpadding="0" cellspacing="1" border="0" width = "100%">
    ...
    while ($sub_products = tep_db_fetch_array($sub_products_query)) {
      ...
      <?php if(tep_not_null($sub_products['products_image']) ){?>
        ...
        document.write('<?php echo '<a href="javascript:popupWindow(\\\'' . tep_href_link(FILENAME_POPUP_IMAGE, 'pID=' . $sub_products['products_id'] . '&image=0') . '\\\')">' . tep_image(DIR_WS_IMAGES . $sub_products['products_image'], addslashes($sub_products['products_name']), ULT_THUMB_IMAGE_WIDTH, ULT_THUMB_IMAGE_HEIGHT, 'hspace="1" vspace="1"') . '<br><span class="enlarge">' . TEXT_CLICK_TO_ENLARGE . '</span></a>'; ?>');
        //--></script>
        <noscript>
        <?php echo '<a href="' . tep_href_link(DIR_WS_IMAGES . $sub_products['products_image']) . '">' . tep_image(DIR_WS_IMAGES . $sub_products['products_image'], $sub_products['products_name'], ULT_THUMB_IMAGE_WIDTH, ULT_THUMB_IMAGE_HEIGHT, 'hspace="1" vspace="1"') . '<br><span class="enlarge">' . TEXT_CLICK_TO_ENLARGE . '</span></a>'; ?>
        </noscript>
      </td>
        <?php } else { ?>
        <td><!-- no image --></td>
        ...
        <td class="productListing-data main" valign="top">
          ...
          if ($sub_products['products_quantity'] > 0 || ($sub_products['products_quantity'] <= 0 && STOCK_ALLOW_CHECKOUT =='true')) {
            if(defined('PRODUCT_INFO_SUB_PRODUCT_PURCHASE') && PRODUCT_INFO_SUB_PRODUCT_PURCHASE == 'Multi'){
              ...
              if(defined('PRODUCT_INFO_SUB_PRODUCT_WISHLIST') && PRODUCT_INFO_SUB_PRODUCT_WISHLIST == 'True'){
                  echo  '<br><input type="button" name="button" id="button" value="Add to Wish List" onclick="sub_products_qty_' . $sub_products['products_id'] . '.value = \'1\'; addwishlist()">';
              }
