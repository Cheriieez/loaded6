...
class upsxml {
    ...
    function upsxml() {
       ...
       // global $shipping_weight, $cart, $shipping_num_boxes;
...
//  =============================  
    function email_errors($vendors_id='1') {  
    ...
    function dimensions_support($vendors_id='1') {  
      
      if (constant('MODULE_SHIPPING_UPSXML_DIMENSIONS_SUPPORTED_' . $vendors_id) == 'Ready-to-ship only') {
        $this->dimensions_support = 1;
      } elseif (constant('MODULE_SHIPPING_UPSXML_DIMENSIONS_SUPPORTED_' . $vendors_id) == 'With product dimensions') {
    ...
    function manual_negotiated_rate($vendors_id='1') {  
      $this->manual_negotiated_rate = constant('MODULE_SHIPPING_UPSXML_RATES_MANUAL_NEGOTIATED_RATE_' . $vendors_id);
      return $this->manual_negotiated_rate;
    } 

    function use_negotiated_rates($vendors_id='1') {  
      $this->use_negotiated_rates = constant('MODULE_SHIPPING_UPSXML_RATES_USE_NEGOTIATED_RATES_' . $vendors_id);
      return $this->use_negotiated_rates;
    }  
    
    function timeInTransitView($vendors_id='1') {  
      $this->timeInTransitView = constant('MODULE_SHIPPING_UPSXML_RATES_TIME_IN_TRANSIT_VIEW_' . $vendors_id);
      return $this->timeInTransitView;
    }

    function pkgvalue($vendors_id='1') {
      // insurance addition
    ...
    function access_account_number($vendors_id='1') {
      $this->access_account_number = constant('MODULE_SHIPPING_UPSXML_RATES_UPS_ACCOUNT_NUMBER_' . $vendors_id);
      return $this->access_account_number;
    } 
    function insure($vendors_id='1') {
      $this->insure = constant('MODULE_SHIPPING_UPSXML_INSURE_' . $vendors_id);
      return $this->insure;
    } 
...
//  =============================  
   ...
   function sort_order($vendors_id='1') {
    ...
    function zones($vendors_id='1') {
      if ( ($this->enabled($vendors_id) == true) && ((int)@constant('MODULE_SHIPPING_UPSXML_RATES_ZONE_' . $vendors_id) > 0) ) {
        $check_flag = false;
...
// you enable dimensional support without changing checkout_shipping.php) but better be safe
        if ($this->dimensions_support($vendors_id) > 0 && is_object($packing)) {
          $boxValue = 0;
          ...
          $boxesToShip = $packing->getPackedBoxes();
             for ($i = 0; $i < count($boxesToShip); $i++) {
                if ($this->insure($vendors_id) == 'False') {
...
// of 100 is used when no insurance is wanted
                  $boxesToShip[$i]['item_price'] = $this->pkgvalue($vendors_id); 
                } 
            ...
            if ($this->insure($vendors_id) == 'False') {
               for ($i = 0; $i < $shipping_num_boxes; $i++) {
                  $this->_addItem(0, 0, 0, $shipping_weight, $this->pkgvalue($vendors_id));
               }
            } else {
...
// of insurance if not divided by the number of boxes
               for ($i = 0; $i < $shipping_num_boxes; $i++) {
                  $this->_addItem(0, 0, 0, $shipping_weight, number_format(($this->pkgvalue($vendors_id)/$shipping_num_boxes), 2, '.', ''));
               }
      ...
      if ($this->timeInTransitView($vendors_id) != 'Not') {
          ...
          if (defined('MODULE_SHIPPING_UPSXML_WEIGHT1') &&  MODULE_SHIPPING_UPSXML_WEIGHT1 == 'False') {
            ...
            for ($i=0; $i < sizeof($upsQuote); $i++) {
                ...
                if ( $method == '' || $method == $type ) {
                    ...
                    if ($this->timeInTransitView($vendors_id) == "Raw") {
                    ...
                    if ( ($this->manual_negotiated_rate($vendors_id) > 0) && ($this->use_negotiated_rates($vendors_id) != 'True') ) {
            $cost = ($this->manual_negotiated_rate($vendors_id) * $cost)/100;
                    }
                    // changed to make handling percentage based
                    if ($this->handling_type($vendors_id) == "Percentage") {
                        $methods[] = array('id' => $type, 'title' => $_type, 'cost' => ((($this->handling_fee($vendors_id) * $cost)/100) + $cost));
                    } else {
    ...
    function _upsGetQuote($vendors_id='1') {
        ...
        "   <Shipment>\n".
        "       <Shipper>\n";
        if ($this->use_negotiated_rates($vendors_id) == 'True') {
        $ratingServiceSelectionRequestHeader .=
        "         <ShipperNumber>" . $this->access_account_number($vendors_id) . "</ShipperNumber>\n";
        }
        $ratingServiceSelectionRequestHeader .=
        "           <Address>\n".
        ...
        for ($i = 0; $i < $this->items_qty; $i++) {
            ...
            "           </PackagingType>\n";
            if ($this->dimensions_support($vendors_id) > 0 && ($this->item_length[$i] > 0 ) && ($this->item_width[$i] > 0 ) && ($this->item_height[$i] > 0)) {
...
/*        $filename = '/srv/www/htdocs/catalog/includes/modules/shipping/example_response.xml';
    ...
    function _post($protocol, $host, $port, $path, $version, $timeout, $xmlRequest) {
        ...
        if (function_exists('exec') && $this->use_exec == '1' ) {
            ...
            if (curl_errno($ch) && $this->email_errors($vendors_id)) {
                $error_from_curl = sprintf('Error [%d]: %s', curl_errno($ch), curl_error($ch));
    ...
    function _parseResult($xmlResult, $vendors_id) {
        ...
        if ($responseStatusCode != '1') {
            ...
            // send email if enabled in the admin section
            if ($this->email_errors($vendors_id)) {
        ...
        if (isset($doc['RatingServiceSelectionResponse']['RatedShipment'][0])) { // more than 1 rate
          for ($i = 0; $i < count($ratedShipments); $i++) {
            ...
            if ($this->use_negotiated_rates($vendors_id) == 'True' && isset($ratedShipments[$i]['NegotiatedRates']['NetSummaryCharges']['GrandTotal']['MonetaryValue'])) {
...
// either a negotiated rate was not given or the shipper does not get/wants any
          ...
          $serviceCode = $ratedShipments['Service']['Code'];
            if ($this->use_negotiated_rates($vendors_id) == 'True' && isset($ratedShipments['NegotiatedRates']['NetSummaryCharges']['GrandTotal']['MonetaryValue'])) {
