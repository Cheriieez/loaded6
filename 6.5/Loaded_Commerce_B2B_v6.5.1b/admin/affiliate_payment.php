...
$action = (isset($_GET['action']) ? $_GET['action'] : '');
  switch ($action) {
    case 'start_billing':
      ...
      // to cut down on the time needed to process various records
      // all zero amount records are simply marked as processed
      $sql = "UPDATE " . TABLE_AFFILIATE_SALES . " set affiliate_billing_status = 1 
              where affiliate_billing_status = 0
                and affiliate_payment = 0
             ";
      tep_db_query ($sql);
       ...
       // Get all orders which are AFFILIATE_BILLING_TIME days old
        ...
        SELECT a.affiliate_orders_id 
          ...
          WHERE a.affiliate_billing_status != 1
            and o.orders_id is NOT NULL
            and o.orders_status = " . AFFILIATE_PAYMENT_ORDER_MIN_STATUS . "
            and a.affiliate_id='" . $affiliate_payment['affiliate_id'] . "'
            and a.affiliate_date <= '" . $oldday . "'
        ";
        ...
        while ($affiliate_orders = tep_db_fetch_array($affiliate_orders_query)) {
          $orders_id_1 .= $affiliate_orders['affiliate_orders_id'] . ",";
        }
        $orders_id .= substr($orders_id_1, 0, -1) .")";
        ...
        $sql="SELECT a.*, c.countries_id, c.countries_name, c.countries_iso_code_2, c.countries_iso_code_3, c.address_format_id 
              FROM " . TABLE_AFFILIATE . " a,
                   " . TABLE_COUNTRIES . " c
              WHERE a.affiliate_id = '" . $affiliate_payment['affiliate_id'] . "'
                and c.countries_id = a.affiliate_country_id
             ";
...
// Notify Affiliate
  ...
  if ( (isset($_GET['action'])) && isset($_GET['pID']) && ($_GET['action'] == 'edit') && tep_not_null($_GET['pID']) ) {
    $pID = tep_db_prepare_input($_GET['pID']);
...
<?php
  if ( isset($_GET['action']) && ($_GET['action'] == 'edit') && isset($payments_exists) ) {
              ...
              <table>
                <tr>
                  <td align="center" valign="bottom" class="main">
                    <?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_PAYMENT, 'pID=' . (isset($pInfo->affiliate_payment_id) ? $pInfo->affiliate_payment_id : ''). '&action=start_billing' ) . '">' . tep_image_button('button_affiliate_billing.gif', IMAGE_AFFILIATE_BILLING) . '</a>'; ?>
                  </td>
                </tr>
              </table>
            </td>
...
<?php
    if (isset($_GET['sID'])) {
      ...
      $payments_query_raw = "select p.* , s.affiliate_payment_status_name 
                             from " . TABLE_AFFILIATE_PAYMENT . " p ,
                                  " . TABLE_AFFILIATE_PAYMENT_STATUS . " s 
...
?>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(array('pID', 'action')) . 'pID=' . $pInfo->affiliate_payment_id . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'magnifier.png', ICON_PREVIEW) . '</a>&nbsp;' . $payments['affiliate_firstname'] . ' ' . $payments['affiliate_lastname']; ?></td>
                ...
                <td class="dataTableContent" align="right"><?php if ( (is_object($pInfo)) && ( $payments['affiliate_payment_id'] == $pInfo->affiliate_payment_id) ) { echo tep_image(DIR_WS_IMAGES . 'arrow_right_blue.png', ''); } else { echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(array('pID')) . 'pID=' . $payments['affiliate_payment_id']) . '">' . tep_image(DIR_WS_IMAGES . 'information.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
