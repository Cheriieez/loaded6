      ...
      for ($i=0; $i<sizeof($order->products); $i++) {
        ...
        for ($ii=0, $n=sizeof($order->products[$i]['orders_products']); $ii<$n; $ii++) {
          ...
          echo '      <td class="dataTableContent" valign="center" align="center">' . $order->products[$i]['orders_products'][$i]['spacer'] . '</td>' . "\n" .
               ...
               '      <td class="dataTableContent" align="center" valign="center"><b>' . $currencies->format($order->products[$i]['orders_products'][$ii]['final_price'], true, $order->info['currency'], $order->info['currency_value']) . '</b></td>' . "\n" .
               '      <td class="dataTableContent" align="center" valign="center"><b>' . $currencies->format(tep_add_tax($order->products[$i]['orders_products'][$ii]['final_price'], $order->products[$i]['orders_products'][$i]['tax']), true, $order->info['currency'], $order->info['currency_value']) . '</b></td>' . "\n" .
               '      <td class="dataTableContent" align="center" valign="center"><b>' . $currencies->format($order->products[$i]['orders_products'][$ii]['final_price'] * $order->products[$i]['orders_products'][$i]['qty'], true, $order->info['currency'], $order->info['currency_value']) . '</b></td>' . "\n" .
               '      <td class="dataTableContent" align="right" valign="center"><b>' .  $currencies->format(tep_add_tax($order->products[$i]['orders_products'][$ii]['final_price'], $order->products[$i]['orders_products'][$i]['tax']) * $order->products[$i]['orders_products'][$i]['qty'], true, $order->info['currency'], $order->info['currency_value']) . '</b></td>' . "\n";
          echo '    </tr>';
        }
      }
      ?>
