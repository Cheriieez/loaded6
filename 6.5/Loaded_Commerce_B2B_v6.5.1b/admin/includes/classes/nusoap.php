  ...
  */
  function invoke_method() {
    ...
    if (strlen($delim) > 0 && substr_count($this->methodname, $delim) == 1 &&
      class_exists(substr($this->methodname, 0, strpos($this->methodname, $delim)), false)) {
...
class nusoap_client extends nusoap_base  {
  ...
  */
  function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
    ...
    $this->debug('SOAP message length=' . strlen($soapmsg) . ' contents (max 2000 bytes)=' . substr($soapmsg, 0, 2000));
    // send
  ...
  function send($msg, $soapaction = '', $timeout=0, $response_timeout=30) {
    ...
    switch(true){
      ...
      case preg_match('/^http/',$this->endpoint):
        ...
        // store raw request/response to session if a session exists
        if (isset($_SESSION)) {
          $_SESSION['nusoap_response'] = $this->response;
          $_SESSION['nusoap_request'] = $this->request;
        }
  ...
  */
    ...
    $this->appendDebug($this->varDump($headers));
    $this->debug('data:');
    $this->appendDebug($this->varDump($data));
    if (!strstr($headers['content-type'], 'text/xml')) {
