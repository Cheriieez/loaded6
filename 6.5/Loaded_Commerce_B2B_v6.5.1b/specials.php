...
*/
  ...
  for ($i = 0, $n = sizeof($column_list); $i < $n; ++$i) {
    switch ($column_list[$i]) {
      ...
      case 'PRODUCT_LIST_PRICE':
        // since we are listing specials, we do not care if this column is selected, it is forced in any case
        break;
    }
  }
  ...
  $listing_sql = "SELECT DISTINCT " . $select_column_list . "
                          ...
                          IF(ISNULL(pg.customers_group_price), LEAST(s.specials_new_products_price, p.products_price), LEAST(pg.customers_group_price, s.specials_new_products_price)) as final_price ";
  // add the tables to be selected from
  if ($need_manufacturer === true) {
    $listing_sql .= "  FROM ((" . TABLE_PRODUCTS . " p
                       LEFT JOIN " . TABLE_SPECIALS . " s on s.products_id = p.products_id and s.customers_group_id = '" . $customer_group_id . "' )
