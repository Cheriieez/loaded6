<?php
/*
  $Id: get1free_productinfotabs_underpriceheading.php,v 1.1.0.0 2008/03/11 13:41:11 maestro Exp $
  ...
  if (defined('MODULE_ADDONS_GET1FREE_STATUS') && MODULE_ADDONS_GET1FREE_STATUS == 'True') {
    ...
    if(defined('GET1FREE_PRODUCTINFO_PLACEMENT') && (GET1FREE_PRODUCTINFO_PLACEMENT == 'underpriceheading')) {
      ...
      if (tep_db_num_rows($get_1_free_query) > 0) {
        ...
        $free_url = '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $free_product['products_free_id'], 'NONSSL') . '" target="_self" title="' . $free_product['products_name'] . '">' . $free_product['products_name'] . '</a>';
        echo '<tr><td align="center" width="99%"><div id="get1free" style="padding:4px; border:1px dashed #ff0000; margin-top:-10px; background-color:pink; font-size:16px;">' . sprintf (TEXT_GET_1_FREE_PROMOTION, $free_product['products_qualify_quantity'], $name['products_name'], $free_product['products_free_quantity'], $free_url) . '</div></td></tr>';
      }
      // end Get 1 Free
    } //end placement check
