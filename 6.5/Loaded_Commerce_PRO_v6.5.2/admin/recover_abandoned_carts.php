		...
		if (defined('RECOVER_CARTS_CHECK_SESSIONS') &&  RECOVER_CARTS_CHECK_SESSIONS == 'True') {
			 if (defined('STORE_SESSIONS') && STORE_SESSIONS == 'mysql') {
				  ...
				  while ($ses = tep_db_fetch_array($sesquery)) {
					   if (preg_match( "/customer_id[^\"]*\"([0-9]*)\"/", $ses['value'], $custval)) $cust_ses_ids[] = $custval[1];
				  }
			 } else {	// --- FILES ---
          ...
          for ($i = 0; $i <= $knt; $i++) {
            ...
            if ($curcus != $inrec['cid']) {
              ...
              if ($curcus != "") {
                ...
                $skip = false;
	               $sentdate = "";
	               $beforeDate = (defined('RECOVER_CARTS_CARTS_MATCH_ALL_DATES') && RECOVER_CARTS_CARTS_MATCH_ALL_DATES == 'True') ? '0' : $inrec['bdate'];
                ...
                // See if the customer has purchased from us before
	               ...
	               $ccquery = tep_db_query("SELECT orders_id, orders_status 
                                           ...
                                           or customers_name like '" . tep_db_input($inrec['fname'] . " " . $inrec['lname']) . "') 
