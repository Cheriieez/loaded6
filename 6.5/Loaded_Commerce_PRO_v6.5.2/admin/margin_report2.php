...
*/
  ...
  // show orders with selected status
	...
	if (isset($_GET['date']) && ($_GET['date'] == 'yes')) {
	  ...
	  $order_query = mysql_query("select orders_id from orders where date_purchased > '" . $_GET['sdate'] . "' and date_purchased < '" . $_GET['edate'] . "' and orders_status = '" . (int)$status . "' order by orders_id asc");
	} else {
	  if ($_GET['report_id'] != '1') {
...
exit();
  ...
  // orders status
	...
	$page_contents .= '<tr><td align="left" class="main"><table><tr><td><a href="' . tep_href_link(FILENAME_MARGIN_REPORT, '', 'NONSSL') . '">' . tep_image_button('button_back.gif', BUTTON_BACK_TO_MAIN) . '</a></td><td>' . tep_draw_form('export_to_file', FILENAME_MARGIN_REPORT2, 'get', '') . tep_draw_hidden_field('action', 'export') . tep_draw_hidden_field('report_id', $_GET['report_id']) . tep_image_submit(DIR_WS_IMAGES . 'export.png', TEXT_EXPORT_BUTTON) . '</form></td></tr></table></td></tr>';
	$page_contents .= '<tr><td>&nbsp;</td></tr>';
	$page_contents .= '<tr><td align="left" class="main"><table><tr><td class="dataTableContent">' . tep_draw_form('export_to_file_by_date', FILENAME_MARGIN_REPORT2, 'get', '') . tep_draw_hidden_field('action', 'export') . tep_draw_hidden_field('date', 'yes') . TEXT_QUERY_DATES.' </td></tr></table><table><tr><td><input type="text" name="sdate" value="' . TEXT_START_DATE . '"></td><td><input type="text" name="edate" value="' . TEXT_END_DATE . '"></td><td>' . tep_image_submit(DIR_WS_IMAGES . 'export.png', TEXT_EXPORT_BUTTON) . '</form></td></tr></table></td></tr>';
?>
