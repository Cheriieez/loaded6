...
<?php
  ...
  while ($orders_status = tep_db_fetch_array($orders_status_query)) {
    if (((!isset($_GET['oID'])) || (isset($_GET['oID']) && $_GET['oID'] == $orders_status['returns_status_id'])) && (!isset($oInfo)) && (isset($_GET['action']) && substr($_GET['action'], 0, 3) != 'new')) {
      $oInfo = new objectInfo($orders_status);
    }

    if ( isset($oInfo) && (is_object($oInfo)) && ($orders_status['returns_status_id'] == $oInfo->returns_status_id) ) {
...
?>
                <td class="dataTableContent" align="right"><?php if ( isset($oInfo) && (is_object($oInfo)) && ($orders_status['returns_status_id'] == $oInfo->returns_status_id) ) { echo tep_image(DIR_WS_IMAGES . 'arrow_right_blue.png', ''); } else { echo '<a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $orders_status['returns_status_id']) . '">' . tep_image(DIR_WS_IMAGES . 'information.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
...
?>
                  <tr>
                    <td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&action=new') . '">' . tep_image_button('button_insert.gif', IMAGE_INSERT) . '</a>'; ?>&nbsp;</td>
                  </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>' . tep_image_submit('button_insert.gif', IMAGE_INSERT));
      break;
    case 'edit':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->returns_status_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>' . tep_image_submit('button_update.gif', IMAGE_UPDATE));
      break;
    case 'delete':
      ...
      if ($remove_status) $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->returns_status_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>' . tep_image_submit('button_delete.gif', IMAGE_DELETE));
      break;
    default:
      if (isset($oInfo) && is_object($oInfo)) {
        $heading[] = array('text' => '<b>' . $oInfo->returns_status_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->returns_status_id . '&action=edit') . '">' . tep_image_button('button_page_edit.png', IMAGE_EDIT) . '</a><a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->returns_status_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
