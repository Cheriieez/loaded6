...
*/
  ...
  include_once(DIR_WS_LANGUAGES . $language . '/' . 'returns.php');
  ...
  switch ($action) {
    case 'update_order':
      ...
      $refund_amount_query = tep_db_query("select r.refund_payment_value, m.customers_email_address, m.customers_name, m.customers_id FROM " . TABLE_RETURN_PAYMENTS . " r, " . TABLE_RETURNS . " m where m.returns_id = r.returns_id and r.returns_id = '" . $oID . "'");
          ...
          $address = strtolower($refund_amount['customers_email_address']);
      //additional query for ccGV System
          $gv_name_query = tep_db_query("select customers_firstname, customers_lastname from " . TABLE_CUSTOMERS . " where lower(customers_email_address) = '" . $refund_amount['customers_email_address'] . "'");
          ...
          $message .= "\n\n" . EMAIL_TEXT_CREDIT_INFORMATION . "\n";
          $message .= TEXT_GV_WORTH  .  $currencies->format($refund) . "\n\n";
          $message .= TEXT_TO_REDEEM;
          $message .= TEXT_WHICH_IS . $id1 . TEXT_IN_CASE . "\n\n";
          $message .= EMAIL_TEXT_CLICKTO_REDEEM . ' ' . "<a href='" . tep_catalog_href_link('gv_redeem.php', 'gv_no=' . $id1,'NONSSL',false) . "'>" . tep_catalog_href_link('gv_redeem.php', 'gv_no=' . $id1,'NONSSL',false) . "</a>\n\n" ;
          ...
          $message .= sprintf(EMAIL_TEXT_GV_INFORMATION,tep_catalog_href_link('gv_faq.php'));
    ...
    // tep_db_query("UPDATE " . TABLE_RETURN_PAYMENTS . " set refund_payment_value = '" . $final_price . "' where returns_id = '" . $_GET['oID'] . "' ");
      ...
      if ($_POST['complete'] == 'on') {
         $notify_comments = '';
         $return_status_id = tep_db_fetch_array(tep_db_query("SELECT returns_status_id FROM " . TABLE_RETURNS_STATUS . " WHERE returns_status_name like '%Complete%'"));
         if ($return_status_id['returns_status_id']) {
          tep_db_query("UPDATE " . TABLE_RETURNS . " set returns_date_finished = now(), date_finished = now(), returns_status = '" . $return_status_id['returns_status_id'] . "' where returns_id = '" . $oID . "'");
        } else {
...
// Remove CVV Number
  ...
  if ( (isset($_GET['action']) && $_GET['action'] == 'edit') && isset($_GET['oID']) ) {
    $oID = tep_db_prepare_input($_GET['oID']);
        ...
        <tr>
          <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td align="right"><?php echo '<a href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('action'))) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
              </tr>
            </table></td>
        </tr>
  <?php
  if ( (isset($_GET['action']) && $_GET['action'] == 'edit') && isset($order_exists) ) {
    $order = new order($oID);
...
?>
      ...
      <?php
      $order_query = tep_db_query("SELECT cc_type, cc_owner, cc_number, cc_expires from " . TABLE_ORDERS . " WHERE orders_id = '" . $oID . "'");
      $order_cc = tep_db_fetch_array($order_query);
      $cc_type = $order_cc['cc_type'];
      $cc_owner = $order_cc['cc_owner'];
      $cc_number = $order_cc['cc_number'];
      $cc_expires = $order_cc['cc_expires'];
      if ( (isset($cc_type) || isset($cc_owner) || isset($cc_number) || isset($cc_expires)) ) {
      ?>
      <tr>
      ...
      <tr>
        ...
        <td class="main"><?php echo $cc_type; ?></td>
      </tr>
      <tr>
        <td class="main"><?php echo ENTRY_CREDIT_CARD_OWNER; ?></td>
        <td class="main"><?php echo $cc_owner; ?></td>
      </tr>
      <tr>
        <td class="main"><?php echo ENTRY_CREDIT_CARD_NUMBER; ?></td>
        <td class="main"><?php echo $cc_number; ?></td>
      </tr>
      <tr>
        <td class="main"><?php echo ENTRY_CREDIT_CARD_EXPIRES; ?></td>
        <td class="main"><?php echo $cc_expires; ?></td>
      </tr>
      <?php
}
?>
...
// for ($i=0; $i<sizeof($order->products); $i++) {
      ...
      if (isset($order->products['attributes']) && (sizeof($order->products['attributes']) > 0)) {
        
        for ($j = 0, $k = sizeof($order->products['attributes']); $j < $k; $j++) {
          echo '<br><nobr><small>&nbsp;<i> - ' . $order->products['attributes'][$j]['option'] . ': ' . $order->products['attributes'][$j]['value'];
          if ($order->products[$i]['attributes'][$j]['price'] != '0') echo ' (' . $order->products['attributes'][$j]['prefix'] . $currencies->format($order->products['attributes'][$j]['price'] * $order->products['qty'], true, $order->info['currency'], $order->info['currency_value']) . ')';
          echo '</i></small></nobr>';
        }
      }
  ...
  <tr>
   ...
   // for ($i=0; $i<sizeof($order->products); $i++) {
      ...
      echo '            </td>' . "\n" .
           '            <td class="dataTableContent" valign="top">' . $order->products['model'] . '</td>' . "\n" .
           '            <td class="dataTableContent" align="right" valign="top">' . tep_display_tax_value($order->products['tax']) . '%</td>' . "\n" .
           '            <td class="dataTableContent" align="right" valign="top"><b>' . $currencies->format($order->products['price'] * $order->products['qty'], true, $order->info['currency'], $order->info['currency_value']) . '</b></td>' . "\n" .
           '            <td class="dataTableContent" align="right" valign="top"><b>' . $currencies->format(tep_add_tax($order->products['final_price'], $order->products['tax']) * $order->products['qty'], true, $order->info['currency'], $order->info['currency_value']) . '</b></td>' . "\n";
      echo '          </tr>' . "\n";
   // }
?>
...
?>
  ...
  <tr>
    <td class="main"><br>
      <b><?php echo TEXT_UPDATE_COMMENTS; ?></b></td>
  </tr>
  <tr>
    <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '5'); ?></td>
  </tr>
    ...
    <tr>
      <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td width=65% class=main><?php echo tep_draw_checkbox_field('restock_charge', '', false); ?>&nbsp;&nbsp;(<?php echo $currencies->format($work_out_charge); ?>)</td>
          </tr>
          <tr>
            <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
...
<tr>
  <td colspan="2" align="right">
    <?php 
    echo  '<a href="javascript: history.go(-1)">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a><a href="' . tep_href_link(FILENAME_RETURNS_INVOICE, 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a><a href="' . tep_href_link("returns_packingslip.php", 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . tep_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a><a href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('action'))) . '"></a>' . tep_image_submit('button_update.gif', IMAGE_UPDATE); 
    ?>
    </form> 
  </td>
</tr>
      ...
      <tr>
        <td align="right"><table border="0" width="100%" cellspacing="0" cellpadding="0">
            ...
            <tr><?php echo tep_draw_form('status', FILENAME_RETURNS, '', 'get'); ?>
              <td class="smallText" align="right"><?php echo HEADING_TITLE_STATUS . ' ' . tep_draw_pull_down_menu('status', array_merge(array(array('id' => '', 'text' => TEXT_ALL_RETURNS)), $orders_statuses), (isset($_GET['status']) ? (int)$_GET['status'] : ''), 'onChange="this.form.submit();"'); ?></td>
              </form>
            </tr>
          </table></td>
      </tr>
...
// NEW "IF ELSEIF ELSE" ADDED BY BURT SO REPLACE THE OLD ONE WITH THIS
     ...
     } elseif (isset($_GET['status']) && (int)$_GET['status'] > 0) {
     ...
     if (isset($_GET['order_id']) && (int)$_GET['order_id'] > 0 && isset($_GET['products_id']) && (int)$_GET['products_id'] > 0) {
          $orders_query_raw = "select o.returns_id, o.returns_status, o.customers_name, o.contact_user_name, o.rma_value, o.payment_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.return_reason_name, op.final_price, o.returns_date_finished, rs.returns_status_name from " . TABLE_RETURNS . " o left join " . TABLE_RETURNS_PRODUCTS_DATA . " op on (o.returns_id = op.returns_id), " . TABLE_RETURN_REASONS . " s " . ", " . TABLE_RETURNS_STATUS . " rs where o.returns_reason = s.return_reason_id and s.language_id = '" . $languages_id . "' and o.order_id = '" . $_GET['order_id'] . "' and op.products_id = '" . $_GET['products_id'] ."' and o.returns_status = rs.returns_status_id order by o.returns_id DESC";
        } else {
...
// END BURT
    ...
    while ($orders = tep_db_fetch_array($orders_query)) {
      if (((!isset($_GET['oID'])) || (isset($_GET['oID']) && $_GET['oID'] == $orders['returns_id'])) && (!isset($oInfo))) {
        $oInfo = new objectInfo($orders);
      }

      if ( isset($oInfo) && (is_object($oInfo)) && ($orders['returns_id'] == $oInfo->returns_id) ) {
    ...
    elseif ($orders['returns_date_finished'] == '0000-00-00 00:00:00' && !isset($return_complete)) {
...
?>
            <td class="dataTableContent" align="center"><b><?php echo $orders['rma_value']; ?></b></a>
              <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders['returns_id'] . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'magnifier.png', ICON_PREVIEW) . '&nbsp;' . $order_done . $return_customer_name . '</font></a>' ; ?></td>
...
?>          </table>
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->returns_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>' . tep_image_submit('button_delete.gif', IMAGE_DELETE));
      break;
    default:
      if (isset($oInfo) && is_object($oInfo)) {
        $heading[] = array('text' => '<b>[' . $oInfo->returns_id . ']&nbsp;&nbsp;' . tep_datetime_short($oInfo->date_purchased) . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->returns_id . '&action=edit') . '">' . tep_image_button('button_page_edit.png', IMAGE_EDIT) . '</a><a href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->returns_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link("returns_packingslip.php", 'oID=' . $oInfo->returns_id) . '" TARGET="_blank">' . tep_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a><a href="' . tep_href_link(FILENAME_RETURNS_INVOICE, 'oID=' . $oInfo->returns_id) . '" TARGET="_blank">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a>');  
        ...
        $contents[] = array('text' => '<br>' . TEXT_INFO_PAYMENT_METHOD . ' <b>'  . tep_date_short($oInfo->returns_date_finished) . '</b>');
      }
      break;
  }
