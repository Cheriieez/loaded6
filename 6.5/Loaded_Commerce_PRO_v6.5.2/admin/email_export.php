...
*/
  ...
  // create sql 
  $sql_string  = "SELECT c.customers_firstname, c.customers_gender, c.customers_lastname, c.purchased_without_account, c.customers_dob, c.customers_email_address, c.customers_newsletter, ci.customers_info_date_account_created, sum( op.final_price * op.products_quantity ) AS order_total, count( o.orders_id ) AS count, max( o.date_purchased ) AS value
  ...
  $sql_string .= " WHERE ci.customers_info_id = c.customers_id AND ci.customers_info_date_account_created BETWEEN '" . tep_db_input(date("Y-m-d H:i:s", $strdate)) . "' AND '" . tep_db_input(date("Y-m-d H:i:s", $enddate)) . "'";
  // Registered Clients
  if($account == 'yes'){
