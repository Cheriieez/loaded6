                    ...
                    //sort view
                       ...
                       switch ($viewedSort) {
                        ...
                        if (isset($_GET['search']) && tep_not_null($_GET['search'])) {
	                        ...
	                        $pending_points_query_raw = "select o.orders_id, 
                                                                o.customers_id, 
                                                                o.customers_name, 
                                                      ...
                                                      left join " . TABLE_ORDERS_TOTAL . " ot 
                                                             on (o.orders_id = ot.orders_id), 
                                                                ...
                                                                " . TABLE_CUSTOMERS_POINTS_PENDING . " pp 
                                                          where o.orders_id = '" . (int)$sorders . "'
                                                            and pp.orders_id = '" . (int)$sorders . "'
                                                            and o.orders_status = s.orders_status_id
                                                            and pp.points_status = '1'
                        ...
                        ?>
                          ...
                          <td class="dataTableContent" align="right"><?php echo $pending_points['orders_status_name']; ?></td>
                          <td class="dataTableContent" align="right"><?php echo $points_status_name; ?></td>
                    ...
                    <tr>
                      <td colspan="7">
                        <table border="0" width="100%" cellspacing="0" cellpadding="2">
                          ...
                          ?>
                          <tr>
                            <td align="right" colspan="2"><?php echo '<a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING) . '">' . tep_image_button('button_reset_filter.gif', IMAGE_RESET) . '</a>'; ?></td>
                          </tr>
                          <?php
                            }
                          ?>
    ...
    default:
      if (isset($uInfo) && is_object($uInfo)) {
		...
		if ($uInfo->points_status == 1) {
          ...
          '<a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=confirm') . '">' . tep_image_button('button_confirm_points.gif', BUTTON_TEXT_CONFIRM_PENDING_POINTS) . '</a> 
          <a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=cancel') . '">' . tep_image_button('button_cancel_points.gif', BUTTON_TEXT_CANCEL_PENDING_POINTS) . '</a> 
          <a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=adjust') . '">' . tep_image_button('button_adjust_points.gif', BUTTON_TEXT_ADJUST_POINTS) . '</a> 
          <a href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $uInfo->customers_email_address) . '">' . tep_image_button('button_email.gif', IMAGE_EMAIL) . '</a> 
          <a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', BUTTON_TEXT_REMOVE_RECORD) . '</a>
          <a href="' . tep_href_link(FILENAME_ORDERS, 'selected_box=orders&oID=' . $uInfo->orders_id . '&action=edit') . '">' . tep_image_button('button_edit_order.gif', IMAGE_EDIT_ORDER) . '</a>');
		}
		if ($uInfo->points_status == 2) {
		  $contents[] = array('align' => 'center', 'text' => 
          '<a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING, tep_get_all_get_params(array('action')) . 'uID=' . $uInfo->unique_id . '&action=rollback') . '">' . tep_image_button('button_rollback_points.gif', BUTTON_TEXT_ROLL_POINTS) . '</a> 
          <a href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $uInfo->customers_email_address) . '">' . tep_image_button('button_email.gif', IMAGE_EMAIL) . '</a> 
          <a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', BUTTON_TEXT_REMOVE_RECORD) . '</a>
          <a href="' . tep_href_link(FILENAME_ORDERS, 'selected_box=orders&oID=' . $uInfo->orders_id . '&action=edit') . '">' . tep_image_button('button_edit_order.gif', IMAGE_EDIT_ORDER) . '</a>');
		}
		if ($uInfo->points_status == 3) {
          ...
          '<a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=confirm') . '">' . tep_image_button('button_confirm_points.gif', BUTTON_TEXT_CONFIRM_PENDING_POINTS) . '</a> 
          <a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=adjust') . '">' . tep_image_button('button_adjust_points.gif', BUTTON_TEXT_ADJUST_POINTS) . '</a> 
          <a href="' . tep_href_link(FILENAME_ORDERS . '?oID=' . $uInfo->orders_id . '&action=edit') . '">' . tep_image_button('button_details.gif', IMAGE_DETAILS) . '</a> 
          <a href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $uInfo->customers_email_address) . '">' . tep_image_button('button_email.gif', IMAGE_EMAIL) . '</a> 
          <a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_PENDING, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', BUTTON_TEXT_REMOVE_RECORD) . '</a>
          <a href="' . tep_href_link(FILENAME_ORDERS, 'selected_box=orders&oID=' . $uInfo->orders_id . '&action=edit') . '">' . tep_image_button('button_edit_order.gif', IMAGE_EDIT_ORDER) . '</a>');
		}
