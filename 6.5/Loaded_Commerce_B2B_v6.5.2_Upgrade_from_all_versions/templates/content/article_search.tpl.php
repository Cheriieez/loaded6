...
<?php
  ...
  $group_access_search_articles = tep_get_access_sql('a.products_group_access', $customer_group_array);
  if (isset($_GET['description'])) {
    $search_query = tep_db_query("select ad.articles_name, a.articles_id, ad.articles_description from " . TABLE_ARTICLES_DESCRIPTION . " ad inner join " . TABLE_ARTICLES . " a on ad.articles_id = a.articles_id where a.articles_status = '1' and ad.language_id = '" . (int)$languages_id . "' and (ad.articles_name like '%" . tep_db_input($akeywords) . "%' or ad.articles_description like '%" . tep_db_input($akeywords) . "%' or ad.articles_head_desc_tag like '%" . tep_db_input($akeywords) . "%' or ad.articles_head_keywords_tag like '%" . tep_db_input($akeywords) . "%' or ad.articles_head_title_tag like '%" . tep_db_input($akeywords) . "%') " . $group_access_search_articles . " order by ad.articles_name ASC");
  }  else {
    $search_query = tep_db_query("select ad.articles_name, a.articles_id, ad.articles_description from " . TABLE_ARTICLES_DESCRIPTION . " ad inner join " . TABLE_ARTICLES . " a on ad.articles_id = a.articles_id where a.articles_status='1' and ad.language_id = '" . (int)$languages_id . "' and (ad.articles_name like '%" . tep_db_input($akeywords) . "%' or ad.articles_head_desc_tag like '%" . tep_db_input($akeywords) . "%' or ad.articles_head_keywords_tag like '%" . tep_db_input($akeywords) . "%' or ad.articles_head_title_tag like '%" . tep_db_input($akeywords) . "%') " . $group_access_search_articles . " order by ad.articles_name ASC");
  }    
    $count=0;
?>
        <table>  
