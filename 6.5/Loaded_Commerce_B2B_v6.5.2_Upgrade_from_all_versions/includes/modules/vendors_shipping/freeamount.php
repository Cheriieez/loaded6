      ...
      if ( ($this->enabled == true) && ((int)constant('MODULE_SHIPPING_FREEAMOUNT_ZONE_' . $vendors_id) > 0) ) {
        ...
        while ($check = tep_db_fetch_array($check_query)) {
          if ($check['zone_id'] < 1) {
            $check_flag = true;
            break;
          }/* $check */elseif ($check['zone_id'] == $order->delivery['zone_id']) {
            $check_flag = true;
            break;
          } //elseif
        }//while
...
// class methods
    function quote($method = '', $module = '', $vendors_id = '1') {
      ...
      */
      ...
      $tmp_final_price = 0;
      foreach($order->products as $xk => $xv) {
        if($xv['vendors_id'] == $vendors_id) {
          $tmp_final_price += $xv['final_price']*$xv['qty'];
        }
      }

      if ($tmp_final_price < @constant('MODULE_SHIPPING_FREEAMOUNT_AMOUNT_' . $vendors_id)) {
