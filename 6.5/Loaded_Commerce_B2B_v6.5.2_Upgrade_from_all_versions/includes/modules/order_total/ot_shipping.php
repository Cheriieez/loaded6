    ...
    function process() {
      ...
      if (tep_not_null($order->info['shipping_method'])) {
        if (defined('MVS_STATUS') && MVS_STATUS == 'true') {          
          ...
          $order->info['tax_groups']["$shipping_tax_description"] += $shipping_tax;
          $order->info['total'] += $shipping_tax;
          if (DISPLAY_PRICE_WITH_TAX == 'true') $order->info['shipping_cost'] += $shipping_tax;
        } else if (isset($GLOBALS[$module]->tax_class) && $GLOBALS[$module]->tax_class > 0) {
