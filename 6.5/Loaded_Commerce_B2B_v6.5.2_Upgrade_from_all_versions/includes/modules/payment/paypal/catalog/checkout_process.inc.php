...
*/
  ...
  global $payment_modules, $shipping_modules, $order, $currencies, $cart,  $ip, $isp,
  ...
  if ((MODULE_ADDONS_POINTS_STATUS == 'True') && (USE_REDEEM_SYSTEM == 'true')) {
    if (($order->info['total'] > 0) && (get_award_discounted($order) == true)) {
      ...
      if ((get_redemption_awards($customer_shopping_points_spending) == true) && ($points_toadd > 0)) {
        tep_add_pending_points($_SESSION['customer_id'], (int)$this->orders_id, $points_toadd, $points_comment, $points_type);
      }
    }
    ...
    if (isset($_SESSION['customer_referral']) && (tep_not_null(USE_REFERRAL_SYSTEM))) {
      ...
      $points_type = 'RF';
      tep_add_pending_points($_SESSION['customer_referral'], (int)$this->orders_id, $points_toadd, $points_comment, $points_type);
    }
    ...
    if ($_SESSION['customer_shopping_points_spending']) {
      ...
      tep_redeemed_points($_SESSION['customer_id'], (int)$this->orders_id, $_SESSION['customer_shopping_points_spending']);
    }
  }
...
/*
// this same code is extracting inventory on return from PP in paypal_notify.php - code here is not needed.
...
*/
    ...
    $sql_data_array = array('orders_id' => (int)$this->orders_id,
                            ...
                            'vendors_id' => $order->products[$i]['vendors_id']);//FOR MVS 
...
// store the session info for notification update - gsb
  ...
  global $affiliate_clientdate, $affiliate_clientbrowser, $affiliate_clientip;
  if (isset($_SESSION['affiliate_ref']) && tep_not_null($_SESSION['affiliate_ref'])) {
    $sql_data_array['affiliate_id'] = $_SESSION['affiliate_ref'];
    $sql_data_array['affiliate_clickthroughs_id'] = $_SESSION['affiliate_clickthroughs_id'];
