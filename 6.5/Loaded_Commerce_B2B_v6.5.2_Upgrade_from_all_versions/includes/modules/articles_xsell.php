...
if ($_GET['articles_id']) {
  if(!isset($_SESSION['sppc_customer_group_id'])) {
    $customer_group_id_arr[] = 'G';
  } else {
    $customer_group_id_arr[] = $_SESSION['sppc_customer_group_id'];
  }
  $group_access_xsell_articles = tep_get_access_sql('a.products_group_access', $customer_group_id_arr);
  
$xsell_query = tep_db_query("select distinct a.products_id, a.products_image, ad.products_name from " . TABLE_ARTICLES_XSELL . " ax, " . TABLE_PRODUCTS . " a, " . TABLE_PRODUCTS_DESCRIPTION . " ad where ax.articles_id = '" . (int)$_GET['articles_id'] . "' and ax.xsell_id = a.products_id and a.products_id = ad.products_id and ad.language_id = '" . (int)$languages_id . "' and a.products_status = '1' " . $group_access_xsell_articles . " order by ax.sort_order asc limit " . MAX_DISPLAY_ARTICLES_XSELL);
